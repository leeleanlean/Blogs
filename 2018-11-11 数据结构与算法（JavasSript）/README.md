...
// 查找
1、数组中哈希值除以数组长度的余数的索引位置
2、如果获取到的数据不匹配，则在匹配到的列表上执行线性搜索
```
...
// 添加
添加节点 - 从顶端开始比较，小于子左树查找，大于子右树查找，直至末尾添加新节点

// 查找
搜索节点 - 从顶端开始比较，小于从子树左侧查找，大于从子树右侧查找，直至匹配到

// 删除
1、没有子树直接删除
2、有一个子树直接删除，将子节点移动至删除位置
3、有二个子树直接删除，从子树中找到最大节点移动至删除位置
```